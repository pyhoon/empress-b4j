...
Public Sub Show (Req As ServletRequest) As String
	Dim CacheName As String = "Topics Page"
	If MC.ExistInCache(App.ctx, CacheName) = False Then
		MC.WriteToCache(App.ctx, CacheName, TopicsPage(Req))
	End If
...
Public Sub Modal (Action As String, Data As Map) As String
	Select Action
		Case "Add"
			Dim CacheName As String = "Topics Add Modal"
			If MC.ExistInCache(App.ctx, CacheName) = False Then
			...
			Return modal1.build
		Case "Edit"
			Dim CacheName As String = "Topics Edit Modal"
			If MC.ExistInCache(App.ctx, CacheName) = False Then
			...
			input1.attr("value", Data.Get("topic_name"))
			Return modal1.build			
		Case "Delete"
			Dim CacheName As String = "Topics Delete Modal"
			If MC.ExistInCache(App.ctx, CacheName) = False Then
			...
			p1.text2($"Delete ${Data.Get("topic_name")}?"$)
			Return modal1.build
		Case Else
			Return ""			
	End Select
...
Public Sub Toast (info As ToastInfo, data As List) As String
	Return MH.Toast("topics-container", TopicsTableFilled(data), info)
End Sub

Public Sub RenderedTable (data As List) As String
	Return TopicsTableFilled(data).build
End Sub

Private Sub TopicsPage (Req As ServletRequest) As MiniHtml
...
Private Sub ContainerContent As MiniHtml
	...
	MH.ButtonAdd("Add Topic", "btn btn-success ml-2", "/hx/topics/add", "#modal-content", "click", "#modal-container", "modal").up(div2)
...
Public Sub TopicsTableFilled (data As List) As MiniHtml
	Dim CacheName As String = "Topics Table"
	If MC.ExistInCache(App.ctx, CacheName) = False Then
		MC.WriteToCache(App.ctx, CacheName, TopicsTable)
	End If
	
	Dim CacheName As String = "Topics Table Row"
	If MC.ExistInCache(App.ctx, CacheName) = False Then
		MC.WriteToCache(App.ctx, CacheName, TopicsTableRow.ConvertToBytes) ' bytes()
	End If
	
	Dim table1 As MiniHtml = MC.ReadFromCache(App.ctx, "Topics Table")
	Dim tbody1 As MiniHtml = table1.ChildByName("tbody")
	tbody1.Children.Clear
	For Each row As Map In data
		Dim tr1 As MiniHtml = MC.ReadFromCache(App.ctx, "Topics Table Row") ' bytes()
		tr1.ChildByIndex(0).text2(row.Get("id"))
		tr1.ChildByIndex(1).text2(row.Get("topic_name"))
		tr1.ChildByIndex(2).ChildByIndex(0).attr("hx-get", "/hx/topics/edit/" & row.Get("id"))
		tr1.ChildByIndex(2).ChildByIndex(1).attr("hx-get", "/hx/topics/delete/" & row.Get("id"))
		tr1.up(tbody1)
	Next
	Return table1
End Sub

Public Sub TopicsTable As MiniHtml
...
Public Sub TopicsTableRow As MiniHtml
	...
	MH.AnchorIcon("edit text-primary mx-2", "/hx/topics/edit/{id}", "Edit", "bi bi-pencil").up(td3)
	MH.AnchorIcon("delete text-danger mx-2", "/hx/topics/delete/{id}", "Delete", "bi bi-trash3").up(td3)
	Return tr1
End Sub
...
Private Sub ModalAdd As MiniHtml
	...
	MH.H5.up(modalHeader).cls("modal-title").text("Add Topic")
...
Private Sub ModalEdit As MiniHtml
	...
	Dim modalHeader As MiniHtml = MH.Div.up(form1).cls("modal-header")
	MH.H5.up(modalHeader).cls("modal-title").text("Edit Topic")
	MH.ButtonClose.up(modalHeader)
...
Private Sub ModalDelete As MiniHtml
	...
	Dim modalHeader As MiniHtml = MH.Div.up(form1).cls("modal-header")
	MH.H5.up(modalHeader).cls("modal-title").text("Delete Topic")
	MH.ButtonClose.up(modalHeader)
