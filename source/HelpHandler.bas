...
Private Sub BuildMethods
	...
	Dim Method As Map = RetrieveMethod("Topics", "GetTopicById (id As Int)")
	Method.Put("Desc", "Read one topic by id")
	Method.Put("Elements", $"["{id}"]"$)
	ReplaceMethod(Method)
	
	Dim Method As Map = RetrieveMethod("Topics", "CreateNewTopic '#POST")
	Method.Put("Desc", "Add new topic")
	Dim FormatMap As Map = CreateMap("topic_name": "topic_name")
	Method.Put("Format", FormatMap.As(JSON).ToString)
	Method.Put("Body", FormatMap.As(JSON).ToString)
	ReplaceMethod(Method)

	Dim Method As Map = RetrieveMethod("Topics", "UpdateTopicById (id As Int) '#PUT")
	Method.Put("Desc", "Update topic by id")
	Method.Put("Elements", $"["{id}"]"$)
	Dim FormatMap As Map = CreateMap("topic_name": "topic_name")
	Method.Put("Format", FormatMap.As(JSON).ToString)
	Method.Put("Body", FormatMap.As(JSON).ToString)
	...
	Dim Method As Map = RetrieveMethod("Topics", "DeleteTopicById (id As Int)")
	Method.Put("Desc", "Delete topic by id")
	Method.Put("Elements", $"["{id}"]"$)
	RemoveMethodAndReAdd(Method)
	
	Dim Method As Map = RetrieveMethod("Pages", "GetPages")
	Method.Put("Desc", "Read all pages")
	ReplaceMethod(Method)
	
	Dim Method As Map = RetrieveMethod("Pages", "GetPageById (id As Int)")
	Method.Put("Desc", "Read one page by id")
	Method.Put("Elements", $"["{id}"]"$)
	ReplaceMethod(Method)

	Dim Method As Map = CreateMethodProperties("Pages", "PostPage")
	Method.Put("Desc", "Add new page")
	Dim Format As String = $"{
  "topic_id": 1,
  "page_slug": "page-slug",
  "page_title": "Page Title",
  "page_body": "Sample body",
  "page_status": 0
}"$
  ...
  "page_status": 0
	...
	Dim Method As Map = RetrieveMethod("Pages", "PutPageById (id As Int)")
	Method.Put("Desc", "Update page by id")
	Dim Format As String = $"{
  "topic_id": 1,
  "page_slug": "page-slug",
  "page_title": "Page Title",
  ...
  "page_status": 0
	...
	Dim Method As Map = RetrieveMethod("Pages", "DeletePageById (id As Int)")
	Method.Put("Desc", "Delete page by id")
	Method.Put("Elements", $"["{id}"]"$)
	ReplaceMethod(Method)
	
	Dim Method As Map = RetrieveMethod("Find", "GetAllPages")
	Method.Put("Desc", "Search all pages")
	ReplaceMethod(Method)
	
	Dim Method As Map = RetrieveMethod("Find", "GetPagesByTopicId (id As Int)")
	Method.Put("Desc", "Search all pages by topic id")
	Method.Put("Elements", $"["pages-by-topic-id", "{id}"]"$)
	ReplaceMethod(Method)
	...
	Method.Put("Desc", "Search pages by keyword")
...
Private Sub GenerateVerbSection (section As VerbSection) As String
	Select section.FileUpload
		...
		Case Else
			...
			Dim strBodyInput As String = $"<p><strong>Body:</strong> <textarea id="body${section.ElementId}" rows="6" class="form-control data-body" style="background-color: #363636; color: white; field-sizing: content; font-size: small">${section.Body}</textarea></p>"$
	End Select
	Return $"
        ...
        <div class="details mb-1">
            <div class="row">
                <div class="col-md-3 p-3">
                    ...
                    <label class="border rounded w-100" style="padding: 5px; font-size: small; white-space: pre-wrap;">${section.Params}</label></p>
